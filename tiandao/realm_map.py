...
---------------------------------------------------------------------------------------------
ตอนแรกสาขาทุกแดนใช้ผังภูมิศาสตร์ร่วมกับแดนเซียนหลัก (place_key = 1) ซึ่งประหยัดมากแต่ทำให้
...
ผังถูกสร้างไว้ให้ **ครบทั้งคลังชื่อ** (108 แดน = 540 แห่ง) ตั้งแต่ตอน import ไม่ใช่เท่าจำนวนแดน
ที่โลกหว่านไว้จริง (C.BRANCH_REALMS = 36) เพราะ places.py/materials.py สร้างตารางของตัวเองจาก
PLACES ตั้งแต่ import แล้ว ต่อทีหลังไม่ได้ ส่วนต่างจึงเป็นความจุที่รอไว้เฉยๆ — และเป็นเหตุผลที่
สาขาซึ่งงอกระหว่างซิมยังต้องใช้ผังร่วมกับแดนเซียนหลัก (ดู worldtree._found_branch)
