                ...
                if city:
                    ...
                    if rng.randint(1, 100) > safety:
                        ...
                        if combat_power >= enemy_power:
                            ...
                            a.insight += 0.5
                            R.cultivate(a, gap)
                            msg = "🦅 [ชัยชนะ] ด้วยกำลังเสริมจากสหาย ท่านสยบศัตรูได้! ได้ +100 เงินและ EXP"
                            # เพิ่มความสัมพันธ์
                            if a.companions:
                                for c in a.companions.keys():
                                    a.companions[c] = min(100, a.companions[c] + 10)
                                msg += " 💞 [ความสัมพันธ์] ทุกคนเชื่อมั่นในฝีมือกันมากขึ้น (+10 Affection)"
                    ...
                    if rng.randint(1, 100) <= jianghu:
                        subs = ["tavern_friend", "buy_weapon", "train"]
                        if a.companions:
                        ...
                        if sub == "drink_tea":
                            ...
                            if choice == "share_gold" and a.money.get(w.wid, 0) >= 50:
                                ...
                                msg = f"🍻 [ความสัมพันธ์] ชวน {comp} ดื่มชาและซื้อของขวัญให้ (-50 เงิน) 💞 ประทับใจมาก (+25 Affection)"
                            elif choice == "talk_martial":
                                a.insight += 0.3
                                R.cultivate(a, gap)
                                a.companions[comp] = min(100, a.companions[comp] + 15)
                                msg = f"🍻 [ความสัมพันธ์] แลกเปลี่ยนเคล็ดวิชากับ {comp} อย่างจริงใจ ได้ EXP และความสนิท (+15 Affection)"
                            else:
                                a.companions[comp] = max(0, a.companions[comp] - 15)
                                msg = f"🍻 [ความสัมพันธ์] นั่งดื่มชากับ {comp} แต่มัวสนใจคัมภีร์จนละเลย 💔 น้อยใจ (-15 Affection)"
                            ...
                            if len(a.companions) < 2:
                                ...
                                if new_f not in a.companions:
                                    ...
                                    msg = f"🤝 [สหายใหม่] {new_f} ยินดีร่วมเดินทางไปกับท่าน! (เริ่มต้นความสนิท 20)"
                                else:
                    ...
                    if rng.randint(1, 100) <= wealth:
                        if a.companions and rng.random() > 0.5:
                            ...
                            if a.money.get(w.wid, 0) >= 40:
                                ...
                                msg = f"🛍️ [ความสัมพันธ์] ยอมจ่าย 40 เงินซื้อสมุนไพรที่ {comp} ชอบ 💞 ประทับใจ (+20 Affection)"
                            else:
                                msg = f"🛍️ [ความสัมพันธ์] {comp} อยากได้สมุนไพรแต่ท่านไม่มีเงินพอซื้อให้"
